...
*/
    ...
    override fun visit(node: ASTNode<out Token>): R {
        return when(node.getToken()) {
            ...
            MetaGrammarInfo.macroBody -> visit_macroBody(node as ASTNode.InnerNode<MetaGrammarInfo.macroBody>)
            MetaGrammarInfo.macroPlus -> visit_macroPlus(node as ASTNode.InnerNode<MetaGrammarInfo.macroPlus>)
    ...
    */
    ...
    macroBody -> defValue macroPlus
    */
    fun visit_macroBody(node: ASTNode.InnerNode<MetaGrammarInfo.macroBody>): R {
        throw IllegalStateException("Unexpected expansion ${node.getToken()} -> ${node.getExpansion()} visited while traversing tree")
    }

    fun visit_macroPlus(node: ASTNode.InnerNode<MetaGrammarInfo.macroPlus>): R {
        return when (val id = node.getExpansion().getId()) {
            0 -> visit_macroPlus_0(node)
            1 -> visit_macroPlus_1(node)
    ...
    */
    fun visit_macroPlus_0(node: ASTNode.InnerNode<MetaGrammarInfo.macroPlus>): R {
        throw IllegalStateException("Unexpected expansion ${node.getToken()} -> ${node.getExpansion()} visited while traversing tree") 
    }
    ...
    macroPlus -> <eps>
    */
    fun visit_macroPlus_1(node: ASTNode.InnerNode<MetaGrammarInfo.macroPlus>): R {
        throw IllegalStateException("Unexpected expansion ${node.getToken()} -> ${node.getExpansion()} visited while traversing tree") 
    }
