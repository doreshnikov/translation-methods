...
class DescriptionBuilderOld(
    private val packageName: String,
    private val objectName: String
) {

    fun walk(tree: ASTNode<*>): String {
...
"""
    ...
    private fun collect(tree: ASTNode<*>): String {
        ...
        collectTokens(root.children.first { it.getToken() == Token.REGISTERED["t"] }, sb)
        collectGrammar(root.children.first { it.getToken() == Token.REGISTERED["g"] }, sb)
        return sb.toString()
    }

    private fun collectTokens(tree: ASTNode<*>, sb: StringBuilder) {
        collectTplus((tree as ASTNode.InnerNode<*>).children.first { it.getToken() == Token.REGISTERED["tPlus"] }, sb)
