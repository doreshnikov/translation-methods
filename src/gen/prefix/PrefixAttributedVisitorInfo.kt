...
object PrefixAttributedVisitorInfo : AttributedVisitorInfo {

    val grammarInfo = PrefixGrammarInfo
    val packageName = "gen.prefix"
    val className = "PrefixAttributedVisitorInfoGenerated"
    ...
    override fun getNodeType(token: Token): String {
        return when (token) {
            is Token.StateToken -> 
                "PrefixAttributedVisitorInfoInnerNode<${getFullName(token)}>"
            is Token.DataToken -> 
                "PrefixAttributedVisitorInfoTerminalNode<${getFullName(token)}>"
            is Token.VariantToken -> 
                "PrefixAttributedVisitorInfoTerminalNode<Token.VariantToken.VariantInstanceToken<${getFullName(token)}>>"
    ...
    */
    fun visit_${token}(node: ${getNodeType(token)}, value: PrefixAttributedVisitorInfoData): PrefixAttributedVisitorInfoData"""
        } else {
            """
    fun visit_${token}(node: ${getNodeType(token)}, value: PrefixAttributedVisitorInfoData): PrefixAttributedVisitorInfoData {
...
${expansions.joinToString("\n") {
    ...
    $token -> $it
    */
    fun visit_${token}_${it.getId()}(node: ${getNodeType(token)}, value: PrefixAttributedVisitorInfoData): PrefixAttributedVisitorInfoData"""
            }}"""
        }
    }
    ...
    override fun getAll(): String {
        return Beautifier.detabify(
            """/**
This code is generated by [${PrefixAttributedVisitorInfo::class.qualifiedName}] derived from [${AttributedVisitorInfo::class.qualifiedName}]
...
interface $className : ${AttributedVisitor::class.simpleName}<PrefixAttributedVisitorInfoData, PrefixAttributedVisitorInfoData> {

/*
${grammarInfo.getGrammar()}
*/

    override fun visit(node: ASTNode<out Token>): PrefixAttributedVisitorInfoData {${getChoiceVisit()}
    }
    
    fun <T : Token> visitTerminal(token: T): PrefixAttributedVisitorInfoData
