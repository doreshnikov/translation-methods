...
object RegexAttributedVisitorInfo : AttributedVisitorInfo {

    val grammarInfo = RegexGrammarInfo
    val packageName = "gen.regex"
    val className = "RegexAttributedVisitorInfoGenerated"
    ...
    override fun getNodeType(token: Token): String {
        return when (token) {
            is Token.StateToken -> 
                "RegexAttributedVisitorInfoInnerNode<${getFullName(token)}>"
            is Token.DataToken -> 
                "RegexAttributedVisitorInfoTerminalNode<${getFullName(token)}>"
            is Token.VariantToken -> 
                "RegexAttributedVisitorInfoTerminalNode<Token.VariantToken.VariantInstanceToken<${getFullName(token)}>>"
    ...
    */
    fun visit_${token}(node: ${getNodeType(token)}, value: RegexAttributedVisitorInfoData): RegexAttributedVisitorInfoData"""
        } else {
            """
    fun visit_${token}(node: ${getNodeType(token)}, value: RegexAttributedVisitorInfoData): RegexAttributedVisitorInfoData {
...
${expansions.joinToString("\n") {
    ...
    $token -> $it
    */
    fun visit_${token}_${it.getId()}(node: ${getNodeType(token)}, value: RegexAttributedVisitorInfoData): RegexAttributedVisitorInfoData"""
            }}"""
        }
    }
    ...
    override fun getAll(): String {
        return Beautifier.detabify(
            """/**
This code is generated by [${RegexAttributedVisitorInfo::class.qualifiedName}] derived from [${AttributedVisitorInfo::class.qualifiedName}]
based on grammar description [${RegexGrammarInfo::class.qualifiedName}] derived from [${GrammarInfo::class.qualifiedName}]
*/
...
interface $className : ${AttributedVisitor::class.simpleName}<RegexAttributedVisitorInfoData, RegexAttributedVisitorInfoData> {

/*
${grammarInfo.getGrammar()}
*/

    override fun visit(node: ASTNode<out Token>): RegexAttributedVisitorInfoData {${getChoiceVisit()}
    }
    
    fun <T : Token> visitTerminal(token: T): RegexAttributedVisitorInfoData
