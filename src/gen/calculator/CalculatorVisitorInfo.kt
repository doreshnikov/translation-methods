...
object CalculatorVisitorInfo : VisitorInfo {

    val grammarInfo = CalculatorGrammarInfo
    val packageName = "gen.calculator"
    val className = "CalculatorVisitorInfoGenerated"
    ...
    override fun getNodeType(token: Token): String {
        return when (token) {
            is Token.StateToken -> 
                "CalculatorVisitorInfoInnerNode<${getFullName(token)}>"
            is Token.DataToken -> 
                "CalculatorVisitorInfoTerminalNode<${getFullName(token)}>"
            is Token.VariantToken -> 
                "CalculatorVisitorInfoTerminalNode<Token.VariantToken.VariantInstanceToken<${getFullName(token)}>>"
    ...
    */
    fun visit_${token}(node: ${getNodeType(token)}): CalculatorVisitorInfoData"""
        } else {
            """
    fun visit_${token}(node: ${getNodeType(token)}): CalculatorVisitorInfoData {
...
${expansions.joinToString("\n") {
    ...
    $token -> $it
    */
    fun visit_${token}_${it.getId()}(node: ${getNodeType(token)}): CalculatorVisitorInfoData"""
            }}"""
        }
    }
    ...
    override fun getAll(): String {
        return Beautifier.detabify(
            """/**
This code is generated by [${CalculatorVisitorInfo::class.qualifiedName}] derived from [${VisitorInfo::class.qualifiedName}]
...
interface $className : ${Visitor::class.simpleName}<CalculatorVisitorInfoData> {

/*
${grammarInfo.getGrammar()}
*/

    override fun visit(node: ASTNode<out Token>): CalculatorVisitorInfoData {${getChoiceVisit()}
    }
    
    fun <T : Token> visitTerminal(token: T): CalculatorVisitorInfoData
